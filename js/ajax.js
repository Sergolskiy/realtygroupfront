...
function get_cards_filtered(obj) {
  ...
  // подставление значаения на просмотр типов карточек
  // let access = JSON.parse(window.localStorage.getItem('access'));
  //
  // if (access.role !== 'ROLE_ADMIN') {
  //   if (access.permissions.hasOwnProperty('cards') &&
  //     access.permissions.cards.hasOwnProperty('seeRent')
  //   ) obj.type = 'rent';
  //
  //   if (access.permissions.hasOwnProperty('cards') &&
  //     access.permissions.cards.hasOwnProperty('seeSale')
  //   ) obj.type = 'sale';
  //
  //   if (access.permissions.hasOwnProperty('cards') &&
  //     access.permissions.cards.hasOwnProperty('seeRent') &&
...
function an_archived_cards(id) {

  return $.ajax({
    url: url_backend + '/api/card-unarchived/' + id,
    type: 'PUT',
    dataType: 'json',
    beforeSend: setHeader,
  })
...
// Проверка активности работы по IP
function check_active_ip() {
  return $.ajax({
    url: url_backend + '/api/settings/list',
    type: 'GET',
    dataType: 'json',
    beforeSend: setHeader,
  })
...
// Изменение активности работы по IP
function change_active_ip(data) {
  return $.ajax({
    url: url_backend + '/api/settings/update',
    type: 'PUT',
    dataType: 'json',
    beforeSend: setHeader,
    data: data
...
function update_access_ip(data, id) {
  return $.ajax({
    url: url_backend + `/api/ip-access/${id}/update`,
    type: 'PUT',
    dataType: 'json',
    beforeSend: setHeader,
    data: data
