...
export class CardsArchiveTable extends React.PureComponent {
    ...
    unArchived = (e) => {
        ...
        if(confirm('Вы уверены что хотите ввостановить?')){
            an_archived_cards(id).done((response) => {
                if(response.card) {
                    document.querySelector('i[data-card-id="'+id+'"]').parentNode.parentNode.remove();
                }
            }).fail(() => {
                alert('Ошибка')
            })
